 ...
 * which is then parsed and converting to a route overlay using some classes created by PhongNX.
 *
 * @author PhongNX
...
public abstract class AbstractRouting extends AsyncTask<Void, Void, List<Route>> {
    protected List<RoutingListener> mRoutingListeners;
    ...
    protected AbstractRouting(RoutingListener listener) {
        this.mRoutingListeners = new ArrayList<RoutingListener>();
        registerListener(listener);
    }

    public void registerListener(RoutingListener mListener) {
        if (mListener != null) {
            mRoutingListeners.add(mListener);
        }
    }

    protected void dispatchOnStart() {
        for (RoutingListener mListener : mRoutingListeners) {
